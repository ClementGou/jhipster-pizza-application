...
type RestOf<T extends ICommande | NewCommande> = Omit<T, 'dateHeureCreation' | 'dateHeureLivraison'> & {
...
export class CommandeService {
  ...
  protected convertDateFromClient<T extends ICommande | NewCommande | PartialUpdateCommande>(commande: T): RestOf<T> {
    return {
      ...commande,
      dateHeureCreation: commande.dateHeureCreation?.toJSON() ?? null,
      dateHeureLivraison: commande.dateHeureLivraison?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCommande: RestCommande): ICommande {
    return {
      ...restCommande,
      dateHeureCreation: restCommande.dateHeureCreation ? dayjs(restCommande.dateHeureCreation) : undefined,
      dateHeureLivraison: restCommande.dateHeureLivraison ? dayjs(restCommande.dateHeureLivraison) : undefined,
    };
  }
